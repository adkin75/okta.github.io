...
```html
<script>
    var orgUrl = 'https://{oktaDomain}.com';
...
| Key                     | Description                                                                                                                                          |
|-------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|
| baseUrl                 | This is your Organization URL in Okta. It will be the URL of the admin pages without the **-admin** for example `https://{oktaDomain}.com`   |
...
$url = 'https://{oktaDomain}.com/oauth2/{authorizationServerId}/v1/token?' . $query;
...
```php
if($res->claims['iss'] != 'https://{oktaDomain}.com/') {
...
$jwk = null;
 ...
 if($kidInCache) {
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'https://{oktaDomain}.com/oauth2/{authorizationServerId}/v1/keys');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_HEADER, 0);
