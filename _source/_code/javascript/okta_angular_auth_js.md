...
For security reasons, browsers make it difficult to make requests to other domains. In this example, we'll make requests from `http://localhost:4200` to `https://{oktaDomain}.com`.

You can configure `https://{oktaDomain}.com` to accept our requests by [enabling CORS for `http://localhost:4200`](/docs/api/getting_started/enabling_cors.html#granting-cross-origin-access-to-websites).
...
export class OktaAuthService {
  ...
  oktaAuth = new OktaAuth({
    url: 'https://{oktaDomain}.com/',
    clientId: '{clientId}',
    issuer: 'https://{oktaDomain}.com/oauth2/{authorizationServerId}',
